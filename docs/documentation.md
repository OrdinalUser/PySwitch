...
- [Graylog Syslog reference](https://graylog.org/post/syslog-protocol-a-reference-guide/)
- [Syslog RFC 5424 specification](https://www.rfc-editor.org/rfc/rfc5424)
- [Syslog server tftpd64](https://pjo2.github.io/tftpd64/)
- [Npcap](https://npcap.com/guide/index.html)
- [Npcap - DeepWiki](https://deepwiki.com/nmap/npcap/3-api-reference)
...
Physical link state changes - cable insertion and removal - are detected through Windows Management Instrumentation (WMI). A dedicated background thread subscribes to modification events on the `Win32_NetworkAdapter` WMI class, polling with a timeout to remain responsive to the application shutdown signal.
...
Syslog functionality is verified using tftpd64 as the syslog server, with messages formatted according to RFC 5424. After consulting with the lab instructor, the host OS network stack can be used for ARP resolution, though the underlying Ethernet2 -> IPv4 -> UDP -> Syslog packet crafting will still be handled by the Software Switch and sent via the interface send loops.
...
- Switch core initialized/shutdown - emitted at CRITICAL on successful initialisation or shutdown of the switch core logic
- Physical link up on slot N - emitted at INFO when a cable is detected on a slot via WMI network adapter monitoring.
- Physical link down on slot N - emitted at INFO when a cable is removed from a slot.
- Interface slot assigned - emitted at INFO when a physical interface is bound to a switch slot.
- Interface slot unassigned - emitted at INFO when a physical interface is removed from a switch slot.
- MAC table clearing and cleaning - emitted at INFO during the mentioned tasks

Syslog can be enabled or disabled and reconfigured (server IP, source IP, port, severity threshold) at runtime through the Services panel in the GUI. Settings are validated and only committed after a test packet is successfully sent towards the configured server, though not if it was delivered.

### Packet structure

A syslog message is carried over standard UDP/IP. The syslog service constructs the full Ethernet II → IPv4 → UDP frame carrying the RFC 5424 payload. The host OS network stack is used for ARP resolution; all lower-layer header fields are set by the switch itself. The encapsulation is: Ethernet II → IPv4 → UDP → RFC 5424 syslog message.

The destination UDP port is 514 by default (configurable). Source port is assigned to sentinel 0. The source and destination IP addresses are configurable through the Services panel. The Ethernet2 mac destination is resolved via Destination IP through the OS network stack and source mac address is fetched from the first available connected physical interface on the switched slots.
...
The syslog payload is an UTF-8 string with the following structure:

- *"<PRI>VERSION TIMESTAMP HOSTNAME APP-NAME PROCID MSGID STRUCTURED-DATA MSG"*
- *"<14>1 2026-03-13T10:26:46Z Skytey PySwitch 66140 - - PySwitch.network.core: Assigning interface slot 1 to Famatech Radmin VPN Ethernet Adapter [xx:xx:xx:xx:xx:xx] ip.ip.ip.ip"*
...
- APP-NAME - application name, in our case `PySwitch`
- PROCID - OS process id
- MSGID, STRUCTURED-DATA - all set to the nil value `-` as these fields are not used

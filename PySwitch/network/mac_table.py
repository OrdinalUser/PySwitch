...
class MACTable:
    ...
    def Learn(self, source_mac: MAC, interface: Virtual.Interface) -> None:
        ...
        if interface.physical is None:
            return
        now = time.monotonic()
        self.mapping[source_mac] = MACTable.Entry(
            source_mac, interface, now + self.configuration.live.core.mac_expiry_s
        )
    ...
    def Clean(self, slot: int) -> None:
        ...
        for mac_source, (mac, interface, timestamp_expiry) in self.mapping.items():
            ...
            if now < timestamp_expiry:
                replacement_map[mac_source] = MACTable.Entry(
                    mac, interface, min(timestamp_expiry, future_map_expiry)
                )

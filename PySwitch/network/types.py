from PySwitch.common import IntEnum, NamedTuple, Optional
...
class Ethernet2:
    ...
    def to_bytes(self, payload: bytes) -> bytes:
        return (
            self.mac_destination.to_bytes()
            + self.mac_source.to_bytes()
...
class UDP:
    ...
    def to_bytes(self, payload: bytes) -> bytes:
        return (
            struct.pack(
                "!HHHH", self.source_port, self.destination_port, 8 + len(payload), 0
            )
            + payload
        )
...
class HTTP:
    ...
    _MAGIC: frozenset = frozenset(m.encode(encoding="ascii") for m in Method) | {
        b"HTTP/"
    }

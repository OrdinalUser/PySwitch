...
from PySwitch.network.interface import (
    InterfaceData,
    InterfaceMetrics,
    Physical,
    Statistics,
...
class Interfaces:
        ...
        *,
        ...
        self.interfaces = [
            Virtual.Interface(
                None,
                InterfaceMetrics(
                    Statistics(configuration.static.metrics.throughput_buffer_size),
                    Statistics(configuration.static.metrics.throughput_buffer_size),
                ),
    ...
    def SlotInUse(self, slot: int):
        if slot >= len(self.interfaces) or slot < 0:
            raise ValueError(
                f"Invalid slot argument, provided {slot}, expected in range 0-{len(self.interfaces) - 1}"
            )
    ...
    def AvailableNICs(
        self, exclude_slot: int | None = None, force_reload: bool = False
        ...
        return [
            nic for nic in GetAllAvailableNICs(force_reload) if nic.name not in assigned
        ]
    ...
    def SendVia(
        ...
        if slot_to != -1:
            ...
            for slot_idx in range(len(self.interfaces)):
                if slot_idx == slot_from or not self.interfaces[slot_idx].connected:
                    continue
...
class Core:
    ...
    def Init() -> Core:
        ...
        instance.processed_frames = BoundedSet(
            instance.configuration.static.core.dedup_last_frames
        )
    ...
    def _process_frame_data(
        self, if_data: InterfaceData, interface: Virtual.Interface
    ) -> None:
    ...
    def OnSlotEvent(self, slot: int, event: Literal["connect", "disconnect"]) -> None:
        ...
        if event == "disconnect":
            ...
            logger.info(
                "Link down on slot %d (%s) [%s]",
                slot,
                iface.physical.name if iface.physical else "?",
                iface.physical.description if iface.physical else "?",
            )
        else:
            logger.info(
                "Link up on slot %d (%s) [%s]",
                slot,
                iface.physical.name if iface.physical else "?",
                iface.physical.description if iface.physical else "?",
            )
    ...
    def FrameHandler(self) -> None:
        while not self.stop_event.is_set():
            try:
                frames = self.ingress_queue.get(
                    timeout=self.configuration.live.core.cleanup_thread_sleep_s
                )
                # print(
                #     f"Processing len={len(frames)} frames; qsize={self.ingress_queue.qsize()}",
                #     end="",
                # )
            except Empty:
                # print("\r", end="")
        ...
        self.listening = Core.ListeningOn(
            ...
            ip=set(
                [
                    ...
                    for interface in self.interfaces.interfaces
                    if interface.physical is not None
                    and interface.physical.mac != ""
                    and interface.physical.ip != ""
                ]
            ),
        )

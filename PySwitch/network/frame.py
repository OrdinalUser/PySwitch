...
from PySwitch.network.types import ARP, HTTP, ICMP, IP4, MAC, TCP, UDP, Ethernet2, IPv4
...
class ProtocolParserEntry(Protocol):
    ...
    def parse(
        data: memoryview,
    ) -> Tuple[Any, Optional[Tuple[Protocols, memoryview]]]: ...
...
class ProtocolParser:
    ...
    @classmethod
    def dispatch(
        cls, data: memoryview, protocol: Protocols
    ) -> Tuple[Any, Optional[Tuple[Protocols, memoryview]]]:
        if parser_fn := cls._registered.get(protocol, None):
            return parser_fn(data)
        else:
            raise ValueError(
                f"Attempting to parse unregistered protocol {protocol.value}={protocol.name}"
            )
...
class Ethernet2Parser(ProtocolParser, protocol=Protocols.Ethernet2):
    ...
    def parse(data: memoryview) -> Tuple[Any, Optional[Tuple[Protocols, memoryview]]]:
        ...
        eth.payload = data[
            14:
        ]  # normally, we would cut off another 4 bytes for frame check sequence..
        ...
        if eth.ether_type in Ethernet2Parser._ETH_ETHERTYPE_TO_PROTOCOLS:
            return (
                eth,
                (
                    Ethernet2Parser._ETH_ETHERTYPE_TO_PROTOCOLS[eth.ether_type],
                    eth.payload,
                ),
            )
        else:
...
class ArpParser(ProtocolParser, protocol=Protocols.Arp):
    ...
    def parse(data: memoryview) -> Tuple[Any, Optional[Tuple[Protocols, memoryview]]]:
        (
            ...
            target_mac,
            target_ip,
        ) = ArpParser._ARP_STRUCT.unpack_from(data)
        ...
        arp.sender_mac = MAC()
        arp.sender_mac.data = sender_mac
        arp.sender_ip = IPv4.from_bytes(sender_ip)
        arp.target_mac = MAC()
        arp.target_mac.data = target_mac
...
class IPv4Parser(ProtocolParser, protocol=Protocols.IPv4):
    ...
    def parse(data: memoryview) -> Tuple[IP4, Optional[Tuple[Protocols, memoryview]]]:
        (
            ...
            src,
            dest,
        ) = IPv4Parser._IP_STRUCT.unpack_from(data)
        ...
        if ip4.protocol in IPv4Parser._IP4_PROTOCOL_TO_PROTOCOLS:
            return (
                ip4,
                (IPv4Parser._IP4_PROTOCOL_TO_PROTOCOLS[ip4.protocol], ip4.payload),
            )
        else:
            return (ip4, None)
...
class UdpParser(ProtocolParser, protocol=Protocols.Udp):
    ...
    def parse(data: memoryview) -> Tuple[UDP, Optional[Tuple[Protocols, memoryview]]]:
        udp = UDP()
        udp.source_port, udp.destination_port, udp.length, udp.checksum = (
            UdpParser._UDP_STRUCT.unpack_from(data)
        )
        udp.payload = data[16:]
        ...
        if udp.destination_port in UdpParser._UDP_PORT_TO_PROTOCOL:
            return (
                udp,
                (UdpParser._UDP_PORT_TO_PROTOCOL[udp.source_port], udp.payload),
            )
        return (udp, None)
...
class TcpParser(ProtocolParser, protocol=Protocols.Tcp):
    ...
    def parse(data: memoryview) -> Tuple[TCP, Optional[Tuple[Protocols, memoryview]]]:
        ...
        ) = TcpParser._TCP_STRUCT1.unpack_from(data, 0)

        dataoffset_reserved, flags, tcp.window, tcp.checksum, tcp.urgent_pointer = (
            TcpParser._TCP_STRUCT2.unpack_from(data, 12)
        )
...
class HttpParser(ProtocolParser, protocol=Protocols.Http):
    ...
    def parse(data: memoryview) -> Tuple[HTTP, Optional[Tuple[Protocols, memoryview]]]:
        http = HTTP()
        first_word = (
            data[:8].tobytes().split(b" ", 1)[0].decode("ascii", errors="replace")
        )
        try:
...
class Frame:
    ...
    def _parse(self) -> None:
        # Read L2
        eth, next_proto_data = Cast(
            Tuple[Ethernet2, Optional[Tuple[Protocols, memoryview]]],
            ProtocolParser.dispatch(self.data, Protocols.Ethernet2),
        )
        ...
        while next_proto_data is not None:
            ...
            try:
                this_header, next_proto = ProtocolParser.dispatch(
                    next_proto_payload, next_proto_type
                )
